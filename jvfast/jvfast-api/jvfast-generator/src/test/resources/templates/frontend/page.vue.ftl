  ...
  <el-card-container>
                      ...
                      >
                        <el-tooltip placement="bottom">
                          ...
                          <el-input
                            ...
                            class="avue-input"
                            :clearable="true"
                            placeholder="请输入对应的${field.comment!}"
                          />
                        </el-tooltip>
                      </el-form-item>
                    </el-col>
    ...
    >
      <div slot="title" class="avue-crud__dialog__header">
        <span class="el-dialog__title">${table.comment!}{{ addViewEditDlgType==='add'? '新增':(addViewEditDlgType==='edit'? '编辑': '查看') }}</span>
      </div>
      <div
        ref="content"
                    ...
                    >
                      ...
                      >
                        <el-tooltip placement="bottom">
                          ...
                          <el-input
                            v-model="addViewEditForm.${field.propertyName}"
                            type="text"
                            :minlength="3"
                            :maxlength="120"
                            :show-word-limit="true"
...
const add${entity}Action = '${package.ModuleName}/${cfg.functionName}/add${entity}'
const update${entity}Action = '${package.ModuleName}/${cfg.functionName}/update${entity}'
const remove${entity}Action = '${package.ModuleName}/${cfg.functionName}/remove${entity}'
const remove${entity}sAction = '${package.ModuleName}/${cfg.functionName}/remove${entity}s'
const get${entity}ByIdAction = '${package.ModuleName}/${cfg.functionName}/get${entity}ById'
const get${entity}PageListAction = '${package.ModuleName}/${cfg.functionName}/page${entity}'
...
export default {
  ...
  data () {
    return {
      ...
      searchFormRules: {
        ...
        <#if field.propertyName=='id'>
        <#else>
        ${field.propertyName}: [{ required: true, message: '请输入${field.comment!}', trigger: 'blur' }]<#if field_has_next>,</#if>
        </#if>
        </#list>
      },
  ...
  methods: {
    ...
    handleBatchDeleteRow () {
      ...
      this.$confirm('确定将当前选择的数据全部删除吗?', {
        ...
        .then(() => {
          ...
          const idParams = { ids }
          this.$store.dispatch(remove${entity}sAction, idParams).then((res) => {
    ...
    fetchTable (page, params = {}) {
      ...
      this.$store.dispatch(get${entity}PageListAction, queryData).then((res) => {
        const resCode = res.code
        const message = res.message
        if (resCode === 0) {
          const data = res.data
    ...
    fetchRowByRowId (row) {
      ...
      this.$store.dispatch(get${entity}ByIdAction, idParams).then((res) => {
        const resCode = res.code
        const message = res.message
        if (resCode === 0) {
    ...
    handleRowSave () {
      ...
      this.$store.dispatch(add${entity}Action, addData).then((res) => {
        const resCode = res.code
        const message = res.message
        if (resCode === 0) {
    ...
    handleRowUpdate () {
      ...
      this.$store.dispatch(update${entity}Action, updateData).then((res) => {
        const resCode = res.code
        const message = res.message
        if (resCode === 0) {
          this.handleAddViewEditDlgClose(this.addViewEditForm)
    ...
    handleRowDeleteShow (row, index) {
      this.$confirm('确定将当前选择的数据删除吗?', {
        ...
        .then(() => {
          this.table.loading = true
          const idParams = { id: row.id }
          this.$store.dispatch(remove${entity}Action, idParams).then((res) => {
            const resCode = res.code
            const message = res.message
            if (resCode === 0) {
              this.handleAddViewEditDlgClose(this.addViewEditForm)
  ...
  head () {
    return {
      title: '${entity}管理 | ' + this.website.name,
      meta: [
        { hid: 'keywords', name: 'keywords', content: this.website.keyword || '' },
        { hid: 'description', name: 'description', content: '${entity}管理' }
      ]
    }
  }
}

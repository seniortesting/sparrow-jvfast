// copy from https://github.com/Kauto/better-sqlite3-helper/blob/master/src/database.js
const path = require('path')
const fs = require('fs')
const Database = require('better-sqlite3')

const sqlFileName = 'sff'
let rootDir = '.'
try {
  rootDir = process.cwd()
} catch (e) {
...
const dbFile = path.resolve(rootDir, `./data/${sqlFileName}.db`)
let instance = null
...
DB.prototype.backup = function (destination, options = {}) {
  return this.connection().backup(
    destination || path.resolve(rootDir, `./data/${sqlFileName}-bak-${Date.now()}.db`),
    options
  )
}
...
function createInsertOrReplaceStatement (
  ...
  if (typeof data[0] !== 'object') {
    throw new TypeError('data does not contain a object')
  }

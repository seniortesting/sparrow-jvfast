  ...
  <el-card-container>
          ...
          >
            <template slot-scope="scope">
              <span>
                ...
                <span v-else-if="['operationType'].includes(column.type)">
                  <el-tag type="success">
                    {{ scope.row[column.prop]===1? '接口': '页面' }}
                  </el-tag>
                </span>
    ...
    >
      <div slot="title" class="avue-crud__dialog__header">
        <span class="el-dialog__title">异常错误日志{{ addViewEditDlgType==='add'? '新增':(addViewEditDlgType==='edit'? '编辑': '查看') }}</span>
      </div>
      <div
        ref="content"

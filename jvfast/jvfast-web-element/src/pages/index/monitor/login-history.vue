  ...
  <el-card-container>
    ...
    >
      <div slot="title" class="avue-crud__dialog__header">
        <span class="el-dialog__title">登录日志{{ addViewEditDlgType==='add'? '新增':(addViewEditDlgType==='edit'? '编辑': '查看') }}</span>
      </div>
      <div
        ref="content"
                    ...
                    >
                      ...
                      >
                        <el-tooltip placement="bottom">
                          ...
                          <el-input
                            v-model="addViewEditForm.ipAddress"
                            type="text"
                            :minlength="3"
                            :maxlength="120"
                            :show-word-limit="true"
                    ...
                    >
                      ...
                      >
                        <el-tooltip placement="bottom">
                          ...
                          <el-input
                            v-model="addViewEditForm.loginLocation"
                            type="text"
                            :minlength="3"
                            :maxlength="120"
                            :show-word-limit="true"
                    ...
                    >
                      ...
                      >
                        <el-tooltip placement="bottom">
                          ...
                          <el-input
                            v-model="addViewEditForm.browser"
                            type="text"
                            :minlength="3"
                            :maxlength="120"
                            :show-word-limit="true"
                    ...
                    >
                      ...
                      >
                        <el-tooltip placement="bottom">
                          ...
                          <el-tag :type="addViewEditForm.loginStatus? 'success': 'danger'">
                            {{ addViewEditForm.loginStatus? '成功': '失败' }}
                          </el-tag>
                        </el-tooltip>
                      </el-form-item>
                    </el-col>
                    ...
                    >
                      ...
                      >
                        <el-tooltip placement="bottom">
                          ...
                          <el-input
                            v-model="addViewEditForm.loginTime"
                            type="text"
                            :minlength="3"
                            :maxlength="120"
                            :show-word-limit="true"
                    ...
                    >
                      ...
                      >
                        <el-tooltip placement="bottom">
                          ...
                          <el-input
                            v-model="addViewEditForm.loginMessage"
                            type="textarea"
                            :minlength="3"
                            :maxlength="120"
                            :rows="5"
...
const addMonitorLoginHistoryAction = 'monitor/loginhistory/addMonitorLoginHistory'
const updateMonitorLoginHistoryAction = 'monitor/loginhistory/updateMonitorLoginHistory'
const removeMonitorLoginHistoryAction = 'monitor/loginhistory/removeMonitorLoginHistory'
const removeMonitorLoginHistorysAction = 'monitor/loginhistory/removeMonitorLoginHistorys'
const getMonitorLoginHistoryByIdAction = 'monitor/loginhistory/getMonitorLoginHistoryById'
const getMonitorLoginHistoryPageListAction = 'monitor/loginhistory/getMonitorLoginHistoryPageList'
...
export default {
  ...
  data () {
    return {
      ...
      addViewEditFormRules: {
        ...
        ipAddress: [{ required: true, message: '请输入登录IP地址', trigger: 'blur' }],
        loginLocation: [{ required: true, message: '请输入登录地点', trigger: 'blur' }],
        browser: [{ required: true, message: '请输入浏览器类型', trigger: 'blur' }],
        os: [{ required: true, message: '请输入操作系统', trigger: 'blur' }],
        loginStatus: [{ required: true, message: '请输入登录状态（0成功 1失败）', trigger: 'blur' }],
        loginTime: [{ required: true, message: '请输入访问时间', trigger: 'blur' }],
        loginMessage: [{ required: true, message: '请输入提示消息', trigger: 'blur' }]
      },
  ...
  methods: {
    ...
    handleBatchDeleteRow () {
      ...
      this.$confirm('确定将选择数据删除?', {
        ...
        .then(() => {
          ...
          const idParams = { ids }
          return this.$store.dispatch(removeMonitorLoginHistorysAction, idParams)
        })
        .then((res) => {
    ...
    fetchTable (page, params = {}) {
      ...
      this.$store.dispatch(getMonitorLoginHistoryPageListAction, queryData).then((res) => {
        const resCode = res.code
        const message = res.message
        if (resCode === 0) {
          const data = res.data
    ...
    fetchRowByRowId (row) {
      ...
      this.$store.dispatch(getMonitorLoginHistoryByIdAction, idParams).then((res) => {
        const resCode = res.code
        const message = res.message
        if (resCode === 0) {
    ...
    handleRowSave () {
      ...
      this.$store.dispatch(addMonitorLoginHistoryAction, this.addViewEditForm).then((res) => {
        const resCode = res.code
        const message = res.message
        if (resCode === 0) {
    ...
    handleRowUpdate () {
      ...
      this.$store.dispatch(updateMonitorLoginHistoryAction, this.addViewEditForm).then((res) => {
        const resCode = res.code
        const message = res.message
        if (resCode === 0) {
          this.handleAddViewEditDlgClose(this.addViewEditForm)
    ...
    handleRowDeleteShow (row) {
      this.$confirm('确定将选择数据删除?', {
        ...
        .then(() => {
          this.table.loading = true
          const idParams = { id: row.id }
          return this.$store.dispatch(removeMonitorLoginHistoryAction, idParams)
        })
        .then((res) => {
          this.table.loading = false
          const resCode = res.code
  ...
  head () {
    return {
      title: '用户登录记录 | ' + this.website.name,
      meta: [
        { hid: 'description', name: 'description', content: '用户登录记录' }
      ]
    }
  }
}

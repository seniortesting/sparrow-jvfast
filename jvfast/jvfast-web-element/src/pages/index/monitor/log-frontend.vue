  ...
  <el-card-container>
    ...
    >
      <div slot="title" class="avue-crud__dialog__header">
        <span class="el-dialog__title">前端日志{{ addViewEditDlgType==='add'? '新增':(addViewEditDlgType==='edit'? '编辑': '查看') }}</span>
      </div>
      <div
        ref="content"
                    ...
                    >
                      ...
                      >
                        <el-tag :type="addViewEditForm.requestStatus===1? 'success': 'danger'">
                          {{ addViewEditForm.loginStatus===1? '成功': '失败' }}
                        </el-tag>
                      </el-form-item>
                    </el-col>
                    <el-col
...
export default {
  ...
  head () {
    return {
      title: '前端日志管理 | ' + this.website.name,
      meta: [
        { hid: 'description', name: 'description', content: '前端日志管理' }
      ]
    }
  }
}

  ...
  <el-card-container>
      ...
      >
        ...
        >
          <template slot-scope="scope">
            ...
            <el-button
              v-if="scope.row.notificationType===1"
              type="text"
              icon="el-icon-edit"
    ...
    >
      <div slot="title" class="avue-crud__dialog__header">
        <span class="el-dialog__title">通知公告{{ addViewEditDlgType==='add'? '新增':(addViewEditDlgType==='edit'? '编辑': '查看') }}</span>
      </div>
      <div
        ref="content"
...
export default {
  ...
  data () {
    return {
      ...
      addViewEditFormRules: {
        notificationType: [{ required: true, message: '请选择通知类型', trigger: 'blur' }],
        userName: [{ required: true, message: '请输入接收者', trigger: 'blur' }],
    ...
    handleRowDeleteShow (row) {
      this.$confirm('确定将选择数据删除?', {
        ...
        .then(() => {
          this.table.loading = true
          const idParams = { id: row.id }
          return this.$store.dispatch(removeSysNotificationAction, idParams)
        })
        .then((res) => {
          this.table.loading = false
          const resCode = res.code
  ...
  head () {
    return {
      title: '公告通知管理 | ' + this.website.name,
      meta: [
        { hid: 'description', name: 'description', content: '公告通知管理' }
      ]
    }
  }
}

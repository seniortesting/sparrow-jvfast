                ...
                <el-group>
                  <div class="avue-form__group">
                    ...
                    >
                      ...
                      >
                        <el-tooltip placement="bottom">
                          ...
                          <el-input
                            v-model="addViewEditForm.postName"
                            type="text"
                            :minlength="3"
                            :maxlength="120"
                    ...
                    >
                      ...
                      >
                        <el-input-number
                          v-model="addViewEditForm.postOrder"
...
const addSysPostAction = 'sys/post/addSysPost'
const updateSysPostAction = 'sys/post/updateSysPost'
const removeSysPostAction = 'sys/post/removeSysPost'
const removeSysPostsAction = 'sys/post/removeSysPosts'
const getSysPostByIdAction = 'sys/post/getSysPostById'
const getSysPostPageListAction = 'sys/post/getSysPostPageList'
...
export default {
  ...
  data () {
    return {
      ...
      addViewEditFormRules: {
        postName: [{ required: true, message: '请输入配置名称', trigger: 'blur' }],
        postOrder: [{ required: true, message: '请输入排列顺序', trigger: 'blur' }],
  ...
  methods: {
    ...
    handleBatchDeleteRow () {
      ...
      this.$confirm('确定将选择数据删除?', {
        ...
        .then(() => {
          ...
          const idParams = { ids }
          return this.$store.dispatch(removeSysPostsAction, idParams)
        })
        .then((res) => {
    ...
    fetchTable (page, params = {}) {
      ...
      this.$store.dispatch(getSysPostPageListAction, queryData).then((res) => {
        const resCode = res.code
        const message = res.message
        if (resCode === 0) {
          const data = res.data
    ...
    fetchRowByRowId (row) {
      ...
      this.$store.dispatch(getSysPostByIdAction, idParams).then((res) => {
        const resCode = res.code
        const message = res.message
        if (resCode === 0) {
    ...
    handleRowSave () {
      ...
      this.$store.dispatch(addSysPostAction, this.addViewEditForm).then((res) => {
        const resCode = res.code
        const message = res.message
        if (resCode === 0) {
    ...
    handleRowUpdate () {
      ...
      this.$store.dispatch(updateSysPostAction, this.addViewEditForm).then((res) => {
        const resCode = res.code
        const message = res.message
        if (resCode === 0) {
          this.handleAddViewEditDlgClose(this.addViewEditForm)
    ...
    handleRowDeleteShow (row) {
      this.$confirm('确定将选择数据删除?', {
        ...
        .then(() => {
          this.table.loading = true
          const idParams = { id: row.id }
          return this.$store.dispatch(removeSysPostAction, idParams)
        })
        .then((res) => {
          this.table.loading = false
          const resCode = res.code
  ...
  head () {
    return {
      title: '岗位管理 | ' + this.website.name,
      meta: [
        { hid: 'description', name: 'description', content: '岗位管理' }
      ]
    }
  }
}

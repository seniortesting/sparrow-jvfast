  ...
  <el-card-container>
    ...
    >
      <div slot="title" class="avue-crud__dialog__header">
        <span class="el-dialog__title">系统设置{{ addViewEditDlgType==='add'? '新增':(addViewEditDlgType==='edit'? '编辑': '查看') }}</span>
      </div>
      <div
        ref="content"
                    ...
                    >
                      ...
                      >
                        <el-tooltip placement="bottom">
                          ...
                          <el-input
                            v-model="addViewEditForm.configName"
                            type="text"
                            :minlength="3"
                            :maxlength="120"
                    ...
                    >
                      ...
                      >
                        <el-tooltip placement="bottom">
                          ...
                          <el-input
                            v-model="addViewEditForm.configKey"
                            type="text"
                            :minlength="3"
                            :maxlength="120"
                            :show-word-limit="true"
                    ...
                    >
                      ...
                      >
                        <el-tooltip placement="bottom">
                          ...
                          <el-input
                            v-model="addViewEditForm.configValue"
                            type="text"
                            :minlength="3"
                            :maxlength="120"
                            :show-word-limit="true"
...
const addSysConfigAction = 'sys/config/addSysConfig'
const updateSysConfigAction = 'sys/config/updateSysConfig'
const removeSysConfigAction = 'sys/config/removeSysConfig'
const removeSysConfigsAction = 'sys/config/removeSysConfigs'
const getSysConfigByIdAction = 'sys/config/getSysConfigById'
const getSysConfigPageListAction = 'sys/config/getSysConfigPageList'
...
export default {
  ...
  data () {
    const validateConfigKey = (rule, value, callback) => {
      if (!value) {
        return callback(new Error('配置键值不能为空'))
      }
      if (!isAlphabets(value)) {
        callback(new Error('配置键值只能是字母或者拼音!'))
      } else {
        callback()
      }
    }
    return {
      ...
      addViewEditFormRules: {
        ...
        configKey: [{ required: true, validator: validateConfigKey, trigger: 'blur' }],
  ...
  methods: {
    ...
    handleBatchDeleteRow () {
      ...
      this.$confirm('确定将选择数据删除?', {
        ...
        .then(() => {
          ...
          const idParams = { ids }
          return this.$store.dispatch(removeSysConfigsAction, idParams)
        })
        .then((res) => {
    ...
    fetchTable (page, params = {}) {
      ...
      this.$store.dispatch(getSysConfigPageListAction, queryData).then((res) => {
        const resCode = res.code
        const message = res.message
        if (resCode === 0) {
          const data = res.data
    ...
    fetchRowByRowId (row) {
      ...
      this.$store.dispatch(getSysConfigByIdAction, idParams).then((res) => {
        const resCode = res.code
        const message = res.message
        if (resCode === 0) {
    ...
    handleRowSave () {
      ...
      this.$store.dispatch(addSysConfigAction, this.addViewEditForm).then((res) => {
        const resCode = res.code
        const message = res.message
        if (resCode === 0) {
    ...
    handleRowUpdate () {
      ...
      this.$store.dispatch(updateSysConfigAction, this.addViewEditForm).then((res) => {
        const resCode = res.code
        const message = res.message
        if (resCode === 0) {
          this.handleAddViewEditDlgClose(this.addViewEditForm)
    ...
    handleRowDeleteShow (row) {
      this.$confirm('确定将选择数据删除?', {
        ...
        .then(() => {
          this.table.loading = true
          const idParams = { id: row.id }
          return this.$store.dispatch(removeSysConfigAction, idParams)
        })
        .then((res) => {
          this.table.loading = false
          const resCode = res.code
  ...
  head () {
    return {
      title: '配置管理 | ' + this.website.name,
      meta: [
        { hid: 'description', name: 'description', content: '配置管理' }
      ]
    }
  }
}
